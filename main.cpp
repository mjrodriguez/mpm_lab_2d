...
int main(int argc, char* argv[]) {
    ...
    unique_ptr<PARTICLES> Particle;
    unique_ptr<SIMULATION_PARAMETERS> SimulationParams;

    try {
        runConfig = LoadConfig(argv[1]);
        Grid = MakeGrid(runConfig.gridType);
        Particle = MakeParticles(runConfig.particlesType);
        SimulationParams = MakeSimulationParameters(runConfig.parametersType);
    ...
    string simulationNumber = runConfig.simulationName;
    string directory = runConfig.outputDirectory;
